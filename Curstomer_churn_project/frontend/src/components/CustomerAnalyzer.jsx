...
import { Sliders, Sparkles, DollarSign, ShieldAlert, CheckCircle2, Copy, User, CreditCard, Shield, Tv } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, Cell, LineChart, Line, CartesianGrid } from 'recharts';

export default function CustomerAnalyzer({ selectedModel }) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white tracking-tight flex items-center space-x-2">
              ...
              <span>Single Customer Predictor & Real-time Analyzer</span>
            </h1>
            <p className="text-sm text-slate-400 mt-1">
              Configure all dataset features below to calculate churn probability, local SHAP drivers, expected survival tenure, and segment-aware AI retention offers.
            </p>
          </div>
          <div className="flex items-center space-x-2">
            <span className="text-xs text-indigo-300 font-mono px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20">
              Complete Feature Schema Enabled
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* Left Column: Full Customer Feature Inputs */}
        <div className="lg:col-span-5 glass-panel p-6 space-y-4">
          
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <h3 className="text-sm font-semibold text-white">1. Configure Customer Details</h3>
            <span className="text-[11px] text-slate-400">All 19 Features</span>
          </div>

          {/* Quick Feature Section Toggles */}
          <div className="flex space-x-1 border-b border-slate-800 pb-2">
            <button
              onClick={() => setActiveSection('demographics')}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                activeSection === 'demographics' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              Profile & Billing
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                activeSection === 'services' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              Telecom Services
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                activeSection === 'addons' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              Add-ons & Security
            </button>
          ...
          {activeSection === 'demographics' && (
            <div className="space-y-3 pt-1">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    className="w-full bg-slate-900 border border-slate-700 text-slate-200 text-xs rounded-xl px-3 py-1.5 focus:outline-none focus:border-indigo-500"
                  >
                ...
                <div className="space-y-1">
                  <label className="text-xs text-slate-400 block">Senior Citizen:</label>
                  <select
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-xs text-slate-400 block">Has Partner:</label>
                  <select
                ...
                <div className="space-y-1">
                  <label className="text-xs text-slate-400 block">Has Dependents:</label>
                  <select
              ...
              {/* Payment Method */}
              <div className="space-y-1">
                <label className="text-xs text-slate-300 font-medium block">Payment Method:</label>
                <select
          ...
          {activeSection === 'services' && (
            <div className="space-y-3 pt-1">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-xs text-slate-400 block">Phone Service:</label>
                  <select
                ...
                <div className="space-y-1">
                  <label className="text-xs text-slate-400 block">Multiple Lines:</label>
                  <select
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-xs text-slate-400 block">Streaming TV:</label>
                  <select
                ...
                <div className="space-y-1">
                  <label className="text-xs text-slate-400 block">Streaming Movies:</label>
                  <select
            ...
            <div className="space-y-3 pt-1">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <button
                    onClick={() => handleInputChange('TechSupport', customer.TechSupport === 'Yes' ? 'No' : 'Yes')}
                    className={`w-full py-2 px-3 rounded-xl text-xs font-semibold border transition ${
                      customer.TechSupport === 'Yes' ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40' : 'bg-slate-900 text-slate-400 border-slate-800'
                    }`}
                  >
                ...
                <div className="space-y-1">
                  ...
                  <button
                    onClick={() => handleInputChange('OnlineSecurity', customer.OnlineSecurity === 'Yes' ? 'No' : 'Yes')}
                    className={`w-full py-2 px-3 rounded-xl text-xs font-semibold border transition ${
                      customer.OnlineSecurity === 'Yes' ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40' : 'bg-slate-900 text-slate-400 border-slate-800'
                    }`}
                  >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-xs text-slate-400 block">Online Backup:</label>
                  <button
                    onClick={() => handleInputChange('OnlineBackup', customer.OnlineBackup === 'Yes' ? 'No' : 'Yes')}
                    className={`w-full py-2 px-3 rounded-xl text-xs font-semibold border transition ${
                      customer.OnlineBackup === 'Yes' ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40' : 'bg-slate-900 text-slate-400 border-slate-800'
                    }`}
                  >
                    {customer.OnlineBackup === 'Yes' ? 'Active ✓' : 'Inactive ✗'}
                  </button>
                </div>

                <div className="space-y-1">
                  <label className="text-xs text-slate-400 block">Device Protection:</label>
                  <button
                    onClick={() => handleInputChange('DeviceProtection', customer.DeviceProtection === 'Yes' ? 'No' : 'Yes')}
                    className={`w-full py-2 px-3 rounded-xl text-xs font-semibold border transition ${
                      customer.DeviceProtection === 'Yes' ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40' : 'bg-slate-900 text-slate-400 border-slate-800'
                    }`}
                  >
                    {customer.DeviceProtection === 'Yes' ? 'Active ✓' : 'Inactive ✗'}
                  </button>
                </div>
              </div>
        ...
        {/* Right Column: Prediction Metrics, SHAP Waterfall & AI Strategy */}
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="glass-card p-5 border border-slate-800 flex flex-col justify-between">
              <span className="text-xs text-slate-400 font-medium">Estimated Preserved CLV</span>
          ...
          {/* Local SHAP Waterfall Breakdown */}
          <div className="glass-panel p-6">
            <h3 className="text-sm font-semibold text-white mb-1">Local SHAP Feature Impact Breakdown</h3>
            <p className="text-xs text-slate-400 mb-4">Positive red bars increase churn risk; negative green bars lower churn risk.</p>
          ...
          {prediction?.survival_curve && (
            <div className="glass-panel p-6">
              <h3 className="text-sm font-semibold text-white mb-1">Estimated Customer Survival Probability (Lifelines Cox PH)</h3>
              <p className="text-xs text-slate-400 mb-4">Projected customer retention over 1 to 72 months</p>

              <div className="h-48 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={prediction.survival_curve}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                    <XAxis dataKey="month" stroke="#64748b" fontSize={11} label={{ value: 'Months', position: 'insideBottom', offset: -5, fill: '#94a3b8', fontSize: 10 }} />
                    <YAxis stroke="#64748b" fontSize={11} unit="%" />
                    <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '8px' }} />
                    <Line type="monotone" dataKey="survival_probability" stroke="#6366f1" strokeWidth={2} name="Survival Prob %" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          {/* Segment-Aware Gemini AI Retention Strategy */}
          <div className="glass-panel p-6 border-indigo-500/30 bg-gradient-to-r from-indigo-950/30 to-purple-950/20">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
              <div>
                ...
                <p className="text-xs text-slate-400">Generates custom discount coupons & ROI estimate for this customer segment.</p>
              </div>
